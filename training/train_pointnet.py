...
from batch_3dmot.models.pointnet import PointNetClassifier
...
parser.add_argument('--version', type=str, help="provide dataset version")
parser.add_argument('--config', type=str, help='Provide a config YAML!', required=True)

# Specific args (namespace: pointnet)
...
parser.add_argument('--feature_transform', help="use feature transform")
...
# Load some predefined seed to guarantee reproducibility
print("Used Seed:", params.pointnet.manual_seed)
random.seed(params.pointnet.manual_seed)
torch.manual_seed(params.pointnet.manual_seed)
...
if params.main.dataset == 'nuscenes':
    ...
    train_dataset = batch_3dmot.utils.dataset.PointCloudDataset(params=params,
                                                                split_name=split_name_train,
                                                                class_dict=class_dict_used)

    val_dataset = batch_3dmot.utils.dataset.PointCloudDataset(params=params,
    ...
    if params.pointnet.shuffle_data:
        np.random.seed(params.pointnet.manual_seed)
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               batch_size=int(params.pointnet.batch_size),
                                               sampler=train_sampler,
                                               num_workers=int(params.pointnet.workers),
                                               collate_fn=batch_3dmot.utils.dataset.collate_lidar)
    ...
    if test_scene_meta_list is not None:
        test_dataset = batch_3dmot.utils.dataset.PointCloudDataset(params=params,
...
batch_3dmot.utils.dataset.check_mkdir(os.path.join(params.paths.models, 'pointnet'))

classifier = PointNetClassifier(k=len(class_dict_used), feature_transform=params.pointnet.feature_transform)
#classifier.load_state_dict(torch.load(os.path.join(params.paths.models, params.pointnet.checkpoint)))

if params.pointnet.checkpoint != '':
    classifier.load_state_dict(torch.load(os.path.join(params.paths.models, params.pointnet.checkpoint)))

batch_3dmot.utils.dataset.check_mkdir(os.path.join(params.paths.models))


optimizer = optim.Adam(classifier.parameters(), lr=params.pointnet.lr, betas=(params.pointnet.beta_lo,
                                                                              params.pointnet.beta_hi))
scheduler = optim.lr_scheduler.StepLR(optimizer,
                                      step_size=params.pointnet.scheduler_step_size,
                                      gamma=params.pointnet.scheduler_gamma)

# Move model to GPU (if CUDA available)
classifier.to(device)

# set up tensorboard writer
pointnet_writer = SummaryWriter(log_dir=os.path.join(params.paths.models,
                                                    'runs/pointnet_%s_%s_%s_%s_%s_%s' %
                                                     (str(datetime.datetime.now()).split('.')[0],
                                                      params.main.version,
                                                      params.main.class_dict,
                                                      params.pointnet.batch_size,
                                                      params.pointnet.lr,
                                                      params.pointnet.num_epochs)))

for key, val in vars(params.pointnet).items():
    pointnet_writer.add_text("pointnet."+str(key), str(val))
    

num_batches_train = len(train_dataset) / params.pointnet.batch_size
num_batches_val = len(val_dataset) / params.pointnet.batch_size


def train(model, device, writer, train_loader, val_loader, log_interval):
    """
    TODO: Define combined train/val method for the pointnet training
    Args:
        model:
        device:
        writer:
    ...
    for epoch in range(params.pointnet.num_epochs):

        train_progress = tqdm(train_loader)
        for i, data in enumerate(train_progress):
            
            pointclouds, targets = data
            pointclouds, targets = pointclouds.to(device), targets.to(device)

            optimizer.zero_grad()
            model = model.train()
            
            pred, trans, trans_feat = model(pointclouds.float())
            loss = torch.nn.functional.nll_loss(pred, targets) / float(params.pointnet.batch_size)

            writer.add_scalar("Loss/train", loss.item(), epoch)
            train_progress.set_description(f"Loss: {loss.item():.4f}")
            
            '''
            if params.pointnet.feature_transform:
            ...
            acc = correct.item() / float(params.pointnet.batch_size)
            
            # show training information in console
            if (i + 1) % log_interval == 0:
                tqdm.write('[%d: %d/%d] %s loss: %f accuracy: %f' % (epoch, i, num_batches_train, 
                                                                     blue('train'), loss.item(), acc))
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(val_progress):
                ...
                model = model.eval()
                pred, _, _ = model(pointclouds.float())
                ...
                # show training information in console
                if (i + 1) % log_interval == 0:
                    tqdm.write('[%d: %d/%d] %s loss: %f accuracy: %f' % (epoch, i, num_batches_val, 
                                                                         blue('val'), loss.item(), acc))
                
        scheduler.step()
        torch.save(classifier.state_dict(),
                   '%s/pointnet_epoch%d_%s_%s.pth' % (os.path.join(params.paths.models, 'pointnet/'),
                                                      epoch,
                                                      params.main.version,
                                                      params.main.class_dict))
...
if __name__ == '__main__':
    train(classifier, device, pointnet_writer, train_loader, val_loader, 100)
